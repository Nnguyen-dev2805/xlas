...
def demonstrate_noise():
    """Demo function"""
    print("\n" + "="*70)
    print(" "*20 + "NOISE GENERATOR DEMONSTRATION")
    print("="*70)
    ...
    print("\n[2] Applying different noise types...")
    
    noisy_sp = NoiseGenerator.salt_and_pepper(test_image, 0.02, 0.02)
    print(f"  ✓ Salt & Pepper noise")
    
    noisy_gaussian = NoiseGenerator.gaussian_noise(test_image, std=25)
    print(f"  ✓ Gaussian noise")
    
    noisy_poisson = NoiseGenerator.poisson_noise(test_image)
    print(f"  ✓ Poisson noise")
    
    blurred_motion = NoiseGenerator.motion_blur(test_image, 15, 45)
    print(f"  ✓ Motion blur")
    
    # Create test suite
    print("\n[3] Creating full test suite...")
    test_set = NoiseTestSuite.create_test_set(test_image, include_blurs=True)
    
    print("\n" + "="*70)
    print(" "*20 + "DEMONSTRATION COMPLETED!")
    print("="*70 + "\n")
